...
1. 得到后继层反传来的残差后, 本层反传给前驱层的残差是什么? (其shape必然仅与本层相关)
...
\begin{cases}
\mathbf \Theta_i \mathbf x_i^{\mathsf T}, \quad \mathbf \Theta_i \in \mathbb{R}^{m \times l},
...
\begin{align}
\nabla_i & = \phi(\nabla_{f_i} f_{i+1}, \nabla_{i+1}^{\mathsf T})
           = \phi(\nabla_{\mathbf x_{i+1}} f_{i+1}, \nabla_{i+1}^{\mathsf T}) \quad \in \mathbb{R}^m\\
...
\phi & =
\begin{cases}
\cdot \text{(matrix-vector product)} & \quad \text{如果第}i+1\text{层是连接层}\\
\circ \text{(entry-wise product)} & \quad \text{如果第}i+1\text{层是激活层, 注意此时}l = m\\
\end{cases}\\
\nabla_{\mathbf x_{i+1}} f_{i+1} & =
\begin{cases}
\mathbf \Theta_{i+1}^{\mathsf T} \quad \in \mathbb{R}^{m \times n}, & \quad \text{如果第}i+1\text{层是连接层}\\
