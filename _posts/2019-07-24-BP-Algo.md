...
\begin{cases}
\mathbf \Theta_i \mathbf x_i^{\mathsf T} + \mathbf b, \quad \mathbf \Theta_i \in \mathbb{R}^{m \times l},
...
\begin{align}
\nabla_i & = \phi((\nabla_{i+1})^{\mathsf T}, \nabla_{f_i} f_{i+1})
           = \phi((\nabla_{i+1})^{\mathsf T}, \nabla_{\mathbf x_{i+1}} f_{i+1}) \quad \in \mathbb{R}^m\\
...
\phi & =
\begin{cases}
\cdot \text{(vector-matrix product)} & \quad \text{如果第}i+1\text{层是连接层}\\
